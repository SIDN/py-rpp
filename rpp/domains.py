...
from rpp.model.rpp.domain import DomainCreateRequest, DomainDeleteRequest, DomainRenewRequest, DomainStartTransferRequest, DomainTransferRequest, DomainUpdateRequest
...
async def do_start_transfer(domainname: str,
    ...
    rpp_request = DomainStartTransferRequest(name=domainname, clTRID=rpp_cl_trid, authInfo=auth_info)
...
async def do_reject_transfer(domainname: str, response: Response,
    ...
    return await do_stop_transfer(TransferOpType.REJECT, domainname, response,
            conn=conn, rpp_cl_trid=rpp_cl_trid, rpp_auth_info=auth_info)
