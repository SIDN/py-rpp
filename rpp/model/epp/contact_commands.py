...
def contact_create(request: ContactCreateRequest) -> Epp:
    ...
    epp_request = Epp(
        command=CommandType(
            create=ReadWriteType(other_element=Create(id=request.card.uid[:8],
                                                      postal_info=[PostalInfoType(type_value="loc",
                                                          name=request.card.name.full,
                                                          org=request.card.organizations['org'].name,
                                                          addr=AddrType(
                                                              street=get_value_by_kind(request.card.addresses.root['addr'].components, "name"),
                                                              city=get_value_by_kind(request.card.addresses.root['addr'].components, "locality"),
                                                              sp=get_value_by_kind(request.card.addresses.root['addr'].components, "region"),
                                                              pc=get_value_by_kind(request.card.addresses.root['addr'].components, "postcode"),
                                                              cc=request.card.addresses.root['addr'].countryCode
                                                          )
                                                      )],
                                                      email=request.card.emails.root['email'].address,
                                                      voice=E164Type(value=request.card.phones.root['voice'].number),
